...
plotCov <- function(df, i){
  ...
  p1 <- p + geom_polygon(color = "gray90", size = 0.01) + coord_equal() + ggtitle(paste(na.omit(df$State),":","Over", i, "per 1000 infected in past 28 days"))
  ...
  p2 <- p1 + scale_fill_gradient2(low = "blue", mid = "purple", high = "red", midpoint = 1, breaks = c(.1, seq(.2, .8, by = .2),  seq(1,1.5,by=.1), 1.9), labels = paste(c(.1, seq(.2, .8, by=.2), seq(1,6, by = 1), 10),"x"), limits = c(.2,1.5))
...
ncol(cov19)
head(county_full)
generState(county_full[county_full$State == "TX",])
```
